...
async function callGemini(article) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (res.status === 429) {
        console.log(`\n    ⏳ Rate limited. Waiting 65s... (attempt ${attempt}/3)`);
        await sleep(65000);
        continue;
      }
      if (res.status >= 500) {
        console.log(`\n    ⏳ Server error. Waiting 15s... (attempt ${attempt}/3)`);
        await sleep(15000);
        continue;
      }
...
async function main() {
  ...
  console.log("\n╔══════════════════════════════════════════╗");
  console.log("║  Samvidhan — Batch Content Generator    ║");
  console.log("╚══════════════════════════════════════════╝\n");
  console.log(`📜 Total articles : ${total}`);
  console.log(`✅ Already done   : ${existing}`);
  console.log(`🔄 Remaining      : ${remaining}`);
  console.log(`⚡ This run limit : ${MAX_PER_RUN}`);
  console.log(`⏱  Est. time      : ~${MAX_PER_RUN * 12} seconds\n`);

  if (remaining === 0) {
    console.log("🎉 ALL ARTICLES ALREADY GENERATED! Nothing to do.\n");
    console.log("🚀 Your app on Vercel is ready to use!");
    return;
  }
  ...
  for (let i = 0; i < ARTICLES.length; i++) {
    ...
    if (generatedThisRun >= MAX_PER_RUN) {
      console.log(`\n⏹  Batch limit of ${MAX_PER_RUN} reached.`);
      console.log(`   Run the workflow again to generate the next ${MAX_PER_RUN} articles.\n`);
      break;
    }
    ...
    const doneTotal = existing + generatedThisRun;
    console.log(`[${doneTotal+1}/${total}] Generating ${label}...`);

    try {
      const data = await callGemini(article);
      fs.writeFileSync(filePath, JSON.stringify(data, null, 2), "utf8");
      generatedThisRun++;
      console.log(`       ✅ Done — ${total - doneTotal - 1} remaining after this batch`);
      // 12 second delay to avoid rate limits
      if (generatedThisRun < MAX_PER_RUN) await sleep(12000);
    } catch (err) {
  ...
  const newTotal = existing + generatedThisRun;
  console.log("\n╔══════════════════════════════════════════╗");
  console.log("║  BATCH COMPLETE                          ║");
  console.log("╚══════════════════════════════════════════╝");
  console.log(`✅ Generated this run : ${generatedThisRun}`);
  console.log(`📊 Total so far       : ${newTotal}/${total}`);
  console.log(`🔄 Still remaining    : ${total - newTotal}`);
  if (failed.length) console.log(`⚠️  Failed this run   : ${failed.join(", ")}`);

  if (newTotal >= total) {
    console.log("\n🎉 ALL 202 ARTICLES COMPLETE! Your app is fully ready!");
  } else {
    console.log(`\n▶  Run the workflow again to continue. ${Math.ceil((total-newTotal)/MAX_PER_RUN)} more runs needed.`);
  }
}
