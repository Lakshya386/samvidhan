...
async function callGemini(article) {
  ...
  // We use 10 second base delay to stay safely under limit
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (res.status === 429) {
        // Rate limited — wait 70 seconds to fully reset the 1-minute window
        console.log(`\n    ⏳ Rate limited. Waiting 70s to reset... (attempt ${attempt}/3)`);
        await sleep(70000);
        continue;
      }

      if (res.status >= 500) {
        console.log(`\n    ⏳ Server error ${res.status}. Waiting 15s... (attempt ${attempt}/3)`);
        await sleep(15000);
        continue;
      }
...
async function main() {
  ...
  console.log(`⏱  Estimated time: ~${Math.ceil((total - existing) * 11 / 60)} minutes\n`);
  ...
  for (let i = 0; i < ARTICLES.length; i++) {
    ...
    process.stdout.write(`\r${bar(done, total)} ${done}/${total}  ${label.padEnd(14)}...`);

    try {
      const data = await callGemini(article);
      fs.writeFileSync(filePath, JSON.stringify(data, null, 2), "utf8");
      done++;
      console.log(`\r${bar(done, total)} ${done}/${total}  ✅ ${label.padEnd(14)}   `);
      // 10 second delay between successful requests = 6 RPM, safely under 15 RPM limit
      await sleep(10000);
    } catch (err) {
  ...
  if (failed.length) {
    ...
    console.log(`   Re-run the workflow to retry failed articles.`);
  }
  console.log(`\n🚀 Done! Vercel will auto-deploy with the new files.\n`);
}
